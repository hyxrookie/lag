...
class PPOTrainer():
    ...
    def ppo_update(self, policy: PPOPolicy, sample):
        # sample 里的数据 shape: (Batch, Chunk_Len, Dim)
        obs_batch, share_obs_batch, actions_batch, masks_batch, active_masks_batch, \
            ...
            rnn_states_actor_batch, rnn_states_critic_batch = sample
        ...
        # --- 关键调用 ---
        # 我们的 Actor 现在直接支持 (Batch, Seq, Dim) 输入
        # rnn_states (Mems) 只需要 Chunk 开头的那个
        ...
        returns_batch = returns_batch.view(-1, 1)
        value_preds_batch = value_preds_batch.view(-1, 1)

        # ... 接下来的 Loss 计算代码几乎可以保持原样，因为已经 Flatten 了 ...
